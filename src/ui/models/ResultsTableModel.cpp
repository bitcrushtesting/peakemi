...
{
    switch (verdict) {
        case Verdict::Pass:     return QObject::tr("Pass");
        case Verdict::Marginal: return QObject::tr("Marginal");
        case Verdict::Fail:     return QObject::tr("Fail");
...
{
    switch (detector) {
        case Detector::Peak:      return QObject::tr("Peak");
        case Detector::QuasiPeak: return QObject::tr("Quasi-peak");
        case Detector::Average:   return QObject::tr("Average");
        case Detector::Rms:       return QObject::tr("RMS");
        case Detector::Sample:    return QObject::tr("Sample");
    }
    return {};
}
...
QVariant ResultsTableModel::data(const QModelIndex& index, int role) const
{
    ...
    if (role == SortRole) {
        switch (index.column()) {
            case FrequencyColumn: return toMegahertz(point->frequency);
            case LevelColumn:     return point->correctedAmplitude;
            case LimitColumn:     return point->limitValue;
            case MarginColumn:    return point->marginDb;
            case DwellColumn:     return static_cast<qint64>(point->dwell.count());
            case BandwidthColumn: return static_cast<qint64>(point->rbw.value());
            default: break;
        }
    }
    ...
    switch (index.column()) {
        case FrequencyColumn: return formatted(toMegahertz(point->frequency), 4);
        case LevelColumn:     return formatted(point->correctedAmplitude, 2);
        case UnitColumn:
            return QString::fromUtf8(amplitudeUnitKey(point->unit).data(),
                                     static_cast<qsizetype>(amplitudeUnitKey(point->unit).size()));
        case LimitColumn:     return formatted(point->limitValue, 2);
        case MarginColumn:    return formatted(point->marginDb, 2);
        case VerdictColumn:   return translatedVerdict(point->verdict);
        case DetectorColumn:  return translatedDetector(point->detector);
        case BandwidthColumn:
            return QStringLiteral("%1 kHz")
                .arg(static_cast<double>(point->rbw.value()) / 1000.0, 0, 'g', 4);
        case DwellColumn:     return QStringLiteral("%1 ms").arg(point->dwell.count());
        case TimestampColumn: return QString::fromStdString(toIso8601(point->measuredAt));
        default: break;
    }
    return {};
}
...
QVariant ResultsTableModel::headerData(int section, Qt::Orientation orientation, int role) const
{
    ...
    switch (section) {
        case FrequencyColumn: return tr("Frequency [MHz]");
        case LevelColumn:     return tr("Level");
        case UnitColumn:      return tr("Unit");
        case LimitColumn:     return tr("Limit");
        case MarginColumn:    return tr("Margin [dB]");
        case VerdictColumn:   return tr("Verdict");
        case DetectorColumn:  return tr("Detector");
        case BandwidthColumn: return tr("RBW");
        case DwellColumn:     return tr("Dwell");
        case TimestampColumn: return tr("Measured at");
        default: break;
    }
    return {};
}
...
{
    const auto existing = std::find_if(m_points.begin(), m_points.end(),
